...
func TestNullableValue(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "unset value",
			nullable:  Type[string]{Valid: false},
			wantValue: nil,
			wantErr:   nil,
		},
	}
...
func TestNullableMarshalJSON(t *testing.T) {
	...
	testCases := []testCase{
		{
			name:         "ValuePresent",
			nullable:     New[int](123),
			expectedJSON: []byte(`123`),
		},
		{
			name:         "ValueNull",
			nullable:     Type[int]{Val: 0, Valid: false},
...
func TestNullableScanWithCustomEnum(t *testing.T) {
	...
	model := TestModel{Field: New(TestEnumA)}
...
func TestValuerAndScanner(t *testing.T) {
	valueNullable1 := Type[testValuerScannerStruct]{
		Val:     testValuerScannerStruct{b: []byte("test output string")},
		Valid:   true,
		Present: true,
	}
	valueNullable2 := Type[testValuerScannerStruct]{
		Valid:   false,
		Present: true,
	}
	...
	var scannerNullable1 Type[testValuerScannerStruct]
	var scannerNullable2 Type[testValuerScannerStruct]

	scannerErr1 := scannerNullable1.Scan(scannerData1)
	assert.NoError(t, scannerErr1)
	assert.Equal(t, Type[testValuerScannerStruct]{
		Present: true,
		Valid:   true,
	...
	var scannerNullableUnsupported Type[testValuerScannerStruct]
...
func TestConvertToDriverValue(t *testing.T) {
	var (
		...
		unknownTypeError unknownType  = map[bool]bool{}
	...
	tests := []struct {
		...
		{"UnknownTypeError", unknownTypeError, nil, true},
...
func TestNullableValue_Uint32(t *testing.T) {
	uint32Val := uint32(12345)
	nullableUint32 := New(uint32Val)

	convertedValue, err := nullableUint32.Value()

	if err != nil {
		t.Fatalf("Type[uint32].Value() returned an error: %v", err)
	}

	if _, ok := convertedValue.(int64); !ok {
		t.Fatalf("Type[uint32].Value() returned a non-int64 type: %T", convertedValue)
	}

	if int64(uint32Val) != convertedValue.(int64) {
		t.Errorf("Type[uint32].Value() returned %v, want %v", convertedValue, uint32Val)
	}
}
