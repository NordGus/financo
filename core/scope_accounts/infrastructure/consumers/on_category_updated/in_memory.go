...
import (
	"financo/core/scope_accounts/application/event_handlers/on_category_updated"
...
func NewInMemory(wg *sync.WaitGroup, payload messages.Updated) {
	...
	err := on_category_updated.New(accountsRepo, transactionsRepo, repo).Handle(payload)
	if err != nil {
		log.Println("something went wrong while handling category updated message.", err)
