package on_savings_goal_marked_as_achieved

import (
	"financo/core/scope_savings_goals/application/event_handlers/on_savings_goal_marked_as_achieved"
...
func NewInMemory(wg *sync.WaitGroup, payload messages.MarkedAsAchieved) {
	defer wg.Done()
	...
	err := on_savings_goal_marked_as_achieved.New(goals, savings, update).Handle(payload)
	if err != nil {
		log.Println("something went wrong while handling savings goal deleted message.", err)
