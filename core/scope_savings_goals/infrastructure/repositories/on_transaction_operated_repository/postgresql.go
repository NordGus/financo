package on_transaction_operated_repository
...
func NewPostgreSQL(db databases.SQLAdapter) repositories.OnTransactionOperated {
	return &postgresql{
		db: db,
	}
}

func (r *postgresql) FindAccounts(
	ctx context.Context, record transaction.Record,
) (models.AccountsForTransaction, error) {
	out := models.AccountsForTransaction{}
	...
	out.Source, err = r.findAccount(ctx, conn, record.SourceID)
	if err != nil {
		return out, err
	}

	out.Target, err = r.findAccount(ctx, conn, record.TargetID)
	if err != nil {
		return out, err
	}
...
func (r *postgresql) FindGoalsForCurrency(
	ctx context.Context, cur currency.Type,
) (models.GoalsAndSavingsForCurrency, error) {
...
func (r *postgresql) findAccount(ctx context.Context, conn *sql.Conn, id int64) (account.Record, error) {
