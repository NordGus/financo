...
func Select(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Has(kindKey) {
		for _, k := range strings.Split(r.URL.Query().Get(kindKey), ",") {
			...
			if err := kindsSelectValidation(kind); err != nil {
				log.Println(err)
	...
	for rows.Next() {
		...
		if child.ID.Valid {
			results[idx].Children = append(results[idx].Children, buildSelectChild(child))
		}
	}
...
func kindsSelectValidation(k account.Kind) error {
...
func buildSelectChild(child Child) Preview {
	return Preview{
		ID:          child.ID.Val,
