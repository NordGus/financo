...
import { ActionsForZodEnum, DESCRIPTION_MAX_LENGTH, NAME_MAX_LENGTH, NAME_MIN_LENGTH } from "./constants";

export const schema = z.object({
  kind: z.union([z.literal("capital"), z.literal("savings")]),
  currency: z.enum(CurrenciesForZodEnum, { error: "invalid option" }),
  color: z.string({ error: "required" }).max(10, { error: "invalid" }),
  ...
  historyAt: z.date().max(new Date()).nullish(),
