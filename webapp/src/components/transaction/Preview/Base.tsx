...
function transactionLine(source: Item, target: Item): { top: Item, bottom: Item } {
    if (source.account.kind === Kind.ExternalIncome) return { top: source, bottom: target }
    if (isDebtAccount(source.account.kind) && isCapitalAccount(target.account.kind))
...
export default function Base({
    ...
    const { top, bottom } = transactionLine(
        { account: source, amount: sourceAmount },
        { account: target, amount: targetAmount }
    )
    ...
    return (
        ...
        >
            <p className="text-xl">{top.account.name}</p>
            <p
                className={
