...
import { createSavingsGoal } from "@api/savings-goals";
...
function CreateForm({ onSetOpenForm: onSetOpenForm }: Props) {
    ...
    const form = useForm<z.infer<typeof schema>>({
        ...
        defaultValues: { currency, target: 0 }
    })

    const onSubmit = async (values: z.infer<typeof schema>) => {
        try {
            const { name, currency, target } = await createSavingsGoal(values)

            toast({
                title: "Created",
                description: `Your goal ${name} to reach ${currencyAmountToHuman(target, currency)} has been created`
            })
            ...
            toast({
                ...
                description: "There was a problem creating the savings goal"
            })
        }
    }
